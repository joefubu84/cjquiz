...
var socketIO = require('socket.io'); //Import classes


var _require = require('./utils/liveGames'),
    LiveGames = _require.LiveGames;

var _require2 = require('./utils/players'),
    Players = _require2.Players;
...
var players = new Players(); //Mongodb setup
...
app.use(express["static"](publicPath)); //Starting server on port 3000
...
io.on('connection', function (socket) {
  ...
  socket.on('host-join', function (data) {
    ...
    MongoClient.connect(url, function (err, db) {
      ...
      dbo.collection('kahootGames').find(query).toArray(function (err, result) {
        if (err) throw err; //A kahoot was found with the id passed in url
        ...
        if (result[0] !== undefined) {
          ...
          console.log('Game Created with pin:', game.pin); //Sending game pin to host so they can display it for players to join
  ...
  socket.on('playerAnswer', function (num) {
    ...
    if (game.gameData.questionLive == true) {
      ...
      MongoClient.connect(url, function (err, db) {
        ...
        dbo.collection("kahootGames").find(query).toArray(function (err, res) {
          if (err) throw err;
          var correctAnswer = res[0].questions[gameQuestion - 1].correct; //Checks player answer with correct answer
  ...
  socket.on('nextQuestion', function () {
    var playerData = players.getPlayers(socket.id); //Reset players current answer to 0
